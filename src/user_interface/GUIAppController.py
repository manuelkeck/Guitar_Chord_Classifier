...
from src.TestVisualization import plot_spectrogram
...
class GUIAppController:
    ...
    def show_spectrogram(self):
        """
        This function calls one (or two) function to show a mel-spectrogram of recorded
        audio.
        :return: None
        """
        if self.latest_audio_path != "":
            print("Showing mel-spectrogram of audio")
            self.add_text("[Spectrogram] Showing mel-spectrogram of audio")
            plot_spectrogram(self.latest_audio_path)
            # plot_spectrogram2(self.latest_audio_path)
        else:
            print("Record audio first.")
            self.add_text("[Spectrogram] Record audio first.")
